...
public:
     ...
     */
    ValueArg(const std::wstring &flag, const std::wstring &name,
             const std::wstring &desc, bool req, T value,
             const std::wstring &typeDesc, Visitor *v = NULL);
    ...
    ValueArg(const std::wstring &flag, const std::wstring &name,
             const std::wstring &desc, bool req, T value,
             const std::wstring &typeDesc, ArgContainer &parser,
             Visitor *v = NULL);
 ...
 */
template <class T>
ValueArg<T>::ValueArg(const std::wstring &flag, const std::wstring &name,
                      const std::wstring &desc, bool req, T val,
                      const std::wstring &typeDesc, Visitor *v)
    ...
    if (argMatches(flag)) {
        ...
        if (Arg::delimiter() != L' ' && value == L"")
            throw(ArgParseException(
                L"Couldn't find delimiter for this argument!", toString()));
 ...
 */
template <class T>
std::wstring ValueArg<T>::shortID(const std::wstring &) const {
    return Arg::shortID(L"<" + _typeDesc + L">");
}
 ...
 */
template <class T>
std::wstring ValueArg<T>::longID(const std::wstring &) const {
    return Arg::longID(L"<" + _typeDesc + L">");
}
