...
public:
    ...
    void setParser(CmdLineInterface &parser) {
        if (_parser) {
            throw SpecificationException(L"Arg group can have only one parser");
        }
    ...
    ArgContainer &add(Arg *arg) {
        if (arg->isRequired()) {
            throw SpecificationException(
                L"Required arguments are not allowed"
                L" in an exclusive grouping.",
                arg->longID());
        }
...
inline ArgContainer &ArgGroup::add(Arg *arg) {
    for (iterator it = begin(); it != end(); it++) {
        if (*arg == **it) {
            throw SpecificationException(
                L"Argument with same flag/name already exists!", arg->longID());
        }
    }
        ...
        if ((*it)->isSet()) {
            if (arg != NULL && !(*arg == **it)) {
                ...
                throw CmdLineParseException(
                    ...
                    flag + L" AND " + (*it)->setBy() + L" provided.");
            }
...
inline const std::wstring ArgGroup::getName() const {
    std::wstring name;
    std::wstring sep = L"{";  // TODO: this should change for
