...
public:
     ...
     */
    MultiArg(const std::wstring &flag, const std::wstring &name,
             const std::wstring &desc, bool req, const std::wstring &typeDesc,
             Visitor *v = NULL);
     ...
     */
    MultiArg(const std::wstring &flag, const std::wstring &name,
             const std::wstring &desc, bool req, Constraint<T> *constraint,
             Visitor *v = NULL);
...
template <class T>
MultiArg<T>::MultiArg(const std::wstring &flag, const std::wstring &name,
                      const std::wstring &desc, bool req,
...
std::wstring MultiArg<T>::shortID(const std::wstring &val) const {
    ...
    return Arg::shortID(L"<" + _typeDesc + L">") + L" ...";
}
...
std::wstring MultiArg<T>::longID(const std::wstring &val) const {
    static_cast<void>(val);  // Ignore input, don't warn
    return Arg::longID(L"<" + _typeDesc + L">") + L"  (accepted multiple times)";
}
    ...
    if (_constraint != NULL)
        if (!_constraint->check(_values.back()))
            throw(CmdLineParseException(
                L"Value '" + val +
                    L"' does not meet constraint: " + _constraint->description(),
                toString()));
}
