...
#include <wtclap/TCLAPConfig.h>
#endif

#include <wtclap/ArgException.h>
#include <wtclap/ArgTraits.h>
#include <wtclap/CmdLineInterface.h>
#include <wtclap/StandardTraits.h>
#include <wtclap/Visitor.h>
...
protected:
     ...
     */
    Arg(const std::wstring &flag, const std::wstring &name,
        const std::wstring &desc, bool req, bool valreq, Visitor *v = NULL);
...
public:
     ...
     */
    static wchar_t blankChar() { return L'\a'; }
...
#define TCLAP_NAMESTARTSTRING L"--"
     ...
     */
    std::wstring getDescription() const { return getDescription(_required); }
     ...
     */
    std::wstring getDescription(bool required) const {
        return (required ? std::wstring(L"(") + _requireLabel + L") " : L"") + _description;
    }
     ...
     */
    const std::wstring &setBy() const { return _setBy; }
 ...
 */
template <typename T>
void ExtractValue(T &destVal, const std::wstring &strVal, ValueLike vl) {
    static_cast<void>(vl);  // Avoid warning about unused vl
    wistringstream is(strVal.c_str());
    ...
    if (is.fail())
        throw(
            ArgParseException(L"Couldn't read argument value "
                              L"from string '" +
                              strVal + L"'"));

    if (valuesRead > 1)
        throw(
            ArgParseException(L"More than one valid value parsed from "
                              L"string '" +
                              strVal + L"'"));
}
 ...
 */
template <typename T>
void ExtractValue(T &destVal, const std::wstring &strVal, StringLike sl) {
...
inline Arg::Arg(const std::wstring &flag, const std::wstring &name,
                const std::wstring &desc, bool req, bool valreq, Visitor *v)
    : _flag(flag),
      _name(name),
      _description(desc),
      _required(req),
    ...
    if (_name != ignoreNameString() &&
        (_flag == Arg::flagStartString() || _flag == Arg::nameStartString() ||
         _flag == L" "))
        throw(SpecificationException(
            L"Argument flag cannot be either '" + Arg::flagStartString() +
                L"' or '" + Arg::nameStartString() + L"' or a space.",
            toString()));
...
inline std::wstring Arg::shortID(const std::wstring &valueId) const {
    ...
    if (_valueRequired) id += std::wstring(1, Arg::delimiter()) + valueId;
...
inline std::wstring Arg::toString() const {
    ...
    if (_flag != L"") s += Arg::flagStartString() + _flag + L" ";
